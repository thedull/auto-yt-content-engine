---
name: youtube-research-from-urls
description: End-to-end research pipeline that, given a list of YouTube URLs (from command-line or JSON file), extracts transcripts, converts them to structured Markdown, uploads everything to a fresh NotebookLM notebook, synthesizes a master summary, and generates a presentation (slide deck) and a video. Use this skill whenever the user wants to research specific YouTube videos they've already identified, rather than discovering videos through search. If the user provides a list of URLs or a JSON file with URLs, trigger this.
---

# YouTube Research from URLs

Given a list of YouTube URLs (provided via command-line arguments or a JSON file), produce a self-contained research folder:

```
<output-root>/
├── videos.json              # metadata for provided URLs
...
This skill chains three existing skills (`youtube-transcript`, `transcript-to-markdown`, `notebooklm`) plus a bundled script. Your job as the invoking agent is orchestration + synthesis. The script does the mechanical parts.

## Ask the user up-front

Before starting, confirm three things (or proceed with defaults if the user gave them in the prompt):

1. **Subject** — the research topic (optional if provided via `--subject` or in JSON file).
2. **Output root** — *optional user-defined directory* (e.g. an Obsidian vault path). If the user doesn't specify one, default to `./research/<subject-slug>/` under the current working directory.
...
### Method 1: Command-line URLs

```
/youtube-research-from-urls --subject "My Research Topic" https://youtube.com/watch?v=abc123 https://youtube.com/watch?v=def456, save to ./my-research
```

### Method 2: JSON file

Create a JSON file with URLs and optional subject:

```json
{
  "subject": "My Research Topic",
  "urls": [
...
The subject is determined in this order:
1. Command-line `--subject` flag (highest priority)
2. `subject` key in JSON file
3. Ask the user (if neither provided)

## Pipeline overview

```
[Stage 1] fetch metadata from URLs → videos.json
...
Every stage is **idempotent** — rerunning the skill on the same URLs/output skips already-completed work. If a stage was interrupted, the skill can resume.

---

## Stage 1 — Fetch metadata from URLs
...
${CLAUDE_PROJECT_DIR}/.claude/skills/youtube-transcript/.venv/bin/python \
  ${CLAUDE_PROJECT_DIR}/.claude/skills/youtube-research-from-urls/scripts/fetch_metadata.py \
...
${CLAUDE_PROJECT_DIR}/.claude/skills/youtube-transcript/.venv/bin/python \
  ${CLAUDE_PROJECT_DIR}/.claude/skills/youtube-research-from-urls/scripts/fetch_metadata.py \
  --file urls.json --out <output-root>/videos.json
```

The script fetches metadata for each URL using yt-dlp and writes a `videos.json` file in the same format as the `youtube-research-pipeline` skill. Stderr shows progress and any failures.

**Skip this stage if `videos.json` already exists** and contains entries for all provided URLs.

If yt-dlp fails with errors, it's usually because YouTube broke the extractor or a video is private/unavailable. Suggest to the user:
...
> **Scope:** N YouTube videos on "<subject>", provided by user.
...
Every stage writes to disk before advancing, and every stage checks for its outputs before running. Re-invoking the skill with the same URLs + output root resumes from the first incomplete stage. Tell the user this if they seem nervous about a long-running Stage 5.
